...
This is a Claude Code skill ([SKILL.md](SKILL.md)) plus a small standalone tool ([mutmatrix.py](mutmatrix.py)) for reviewing someone else's pull request by measuring the one thing you cannot get by reading it:
*which of the guards this PR adds would survive being deleted?*

[mutmatrix.py](mutmatrix.py) neutralises each guard in turn, runs the project's own test suite, and records what nothing notices. Survivors are the finding. Everything else in the review — the suggested test, the argument, the ranking — hangs off that table, and [SKILL.md](SKILL.md) is the procedure that turns it into a review.
...
The title of [adk-python#6957](https://github.com/google/adk-python/pull/6957) named three endpoints. One of them was actually guarded by a test.
Reverting the `/run` line alone left the entire suite of [adk-python#6957](https://github.com/google/adk-python/pull/6957) green.

---

## Why it refuses to guess

Most of [mutmatrix.py](mutmatrix.py) is not the mutation loop — that part is twenty lines. Most of it is the checks in [mutmatrix.py](mutmatrix.py) that stop it printing a table it cannot stand behind.
...
Exit `0` means no survivors. Exit `1` from [mutmatrix.py](mutmatrix.py) means survivors — that is your review.
Exit `3` means [mutmatrix.py](mutmatrix.py) measured nothing and you should not quote it.
...
and both were scored `killed`. The review of [adk-python#6957](https://github.com/google/adk-python/pull/6957) would otherwise have certified coverage nobody had. Now:
...
the delete root guard"*. In [anthropic-sdk-python#1906](https://github.com/anthropics/anthropic-sdk-python/pull/1906) the memory tool guarded its root directory with `if command.path == "/memories"`, comparing the raw string the model sent, so `/memories/`, `/memories/.` and `/memories/subdir/..` all reached `shutil.rmtree`.
...
**Step 3, neutralise both guards** (sync and async) with [mutmatrix.py](mutmatrix.py), one at a time:
...
Both guards are real. Six named tests in [anthropic-sdk-python#1906](https://github.com/anthropics/anthropic-sdk-python/pull/1906) die when the sync guard goes:
...
tests caught it, here are the six by name" is worth more to a maintainer than another opinion about style, which is why [SKILL.md](SKILL.md) keeps that verdict.

**Step 0 of [SKILL.md](SKILL.md) is where the finding came from.** Reading the whole file rather than the
...
That became the one actionable finding in [anthropic-sdk-python#1906](https://github.com/anthropics/anthropic-sdk-python/pull/1906), and it exists because of step 0, not because of the mutation table.

## Where the method comes from

The table at the top of this README is that run against [adk-python#6957](https://github.com/google/adk-python/pull/6957). Five of eight redaction sites in [adk-python#6957](https://github.com/google/adk-python/pull/6957) survived, including two of the three endpoints its own title claimed to fix.
The author's reply:
...
— [prasanna8585, 1 Sep 2026](https://github.com/google/adk-python/pull/6957#issuecomment-5489421299)

That review was hand-rolled. This repository is that procedure made repeatable, as [SKILL.md](SKILL.md) and [mutmatrix.py](mutmatrix.py).
...
Then ask Claude to review a PR; [SKILL.md](SKILL.md) triggers on "review this PR", "is this change actually tested?", "mutation test this".
...
- **Bytecode caching will lie to you**, so [mutmatrix.py](mutmatrix.py) defends against it. CPython validates a cached `.pyc` by
  source size and mtime *in whole seconds*. Neutralising the same guard at two call sites usually yields two files of identical length — so two mutants inside one second run the *first* one's bytecode and both score `SURVIVED`, a trap pinned by [_test_mutmatrix.py](_test_mutmatrix.py).
